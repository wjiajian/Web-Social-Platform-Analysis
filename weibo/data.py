...
def get_session():
    try:
        db = pymysql.connect(host='localhost', user='root', passwd='pwd', port=3306)
        db.autocommit(True)
        print('连接成功！')
    except ValueError:
...
class MysqlOp:
    def __init__(self,host='localhost', user='root', passwd='pwd', port=3306,autocommit=True):
        self.db = pymysql.connect(host='localhost', user='root', passwd='pwd', port=3306)
